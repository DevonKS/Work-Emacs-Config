...
(defun customize-file-name (path)
  "Put Filename at the start of string for easier selection"
  (concat (nth 0 (last (split-string path "/"))) " " path))
...
(defun init-open-recentf-buffer-files ()
  ...
  (let ((found-files '()))
    ...
    (setq found-files (mapcar 'customize-file-name found-files))))
